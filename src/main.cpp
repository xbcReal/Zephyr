...
vector<int> MM_COEFF_MODULI = {50, 40, 50};
...
void MM_test() {
    ...
    EncryptionParameters parms(scheme_type::ckks);
    parms.set_poly_modulus_degree(poly_modulus_degree);
    //这是一个模数链，MM_COEFF_MODULI是一个素数幂的选取
    ...
    for (int i = 0; i < (logN)/2; i++) {
        rots.push_back((poly_modulus_degree / exponentiate_uint(2, 2*i))+1);
        rots.push_back((poly_modulus_degree / exponentiate_uint(2, 2*i+1))+1);
        rots.push_back((3*poly_modulus_degree / exponentiate_uint(2, 2*i+1))+1);
    }

    keygen.create_galois_keys(rots, galois_keys);

    std::cout << "The length of rots is: " << rots.size() << std::endl;
    ...
    std::vector<std::vector<double>> matrix_768x768 = mme.readMatrix("../data/input/matrix_768x768.txt", 768, 768);

    auto matrix_4096x768_T = mme.transposeMatrix(matrix_4096x768);
    auto matrix_768x768_T = mme.transposeMatrix(matrix_768x768);

    std::vector<std::vector<double>> row_pack;
    std::vector<double> row_ct(poly_modulus_degree, 0.0);
    for (auto i = 0; i < 768 * 768; i++) {
        int row = i / 768;
        int col = i % 768;
        row_ct[i % poly_modulus_degree] = matrix_768x768_T[row][col];//看需要多少个多项式
        if (i % poly_modulus_degree == (poly_modulus_degree - 1)) {
            row_pack.push_back(row_ct);
        }//这个就是看最后的多项式有没有被填满，以这个设置来看是填满的
    }

    vector<Ciphertext> res;
    auto start = high_resolution_clock::now();
    mme.matrix_mul(matrix_4096x768_T, row_pack, res);//明文密文矩阵相乘，前者128x768是明文，后者row_pack是密文
    auto end = high_resolution_clock::now();
    cout << "[MatMul] 4096x768 x 768x768 takes: " << duration_cast<milliseconds>(end - start).count() << " milliseconds"
         << endl;

    std::vector<std::vector<double>> matrix_4096x768_out = mme.readMatrix("../data/calibration/matrix_result_m_4096_k_768.txt", 4096, 768);
    auto matrix_4096x768_T_out = mme.transposeMatrix(matrix_4096x768_out);
...
//         ckks_evaluator.re_encrypt(res[i]);}
//       auto end2 = high_resolution_clock::now();
//       cout << "[Re-encrypt] 64 takes: " << duration_cast<milliseconds>(end2 - start2).count() << " milliseconds"
//        << endl;
//       size_t res_memory = 0;
//       for (const auto& ct : res) {
//         res_memory += ct.size();
//         }
//       cout << "Memory usage of re-encrypted res: " << res_memory << " bytes" << endl;
      Plaintext res_pt;
      ...
      cout << "[decrypt] 64 takes: " << duration_cast<milliseconds>(end3 - start3).count() << " milliseconds"
       << endl;

      for (auto i = 0; i < 2; i++) {
        average_err += fabs(mm_res[i] / 2.0 - matrix_4096x768_T_out[0][i]);
      }

      std::cout << "Average error: " << average_err / 2.0 << std::endl;
//     double average_err = 0.0;
//     for (uint32_t j = 0; j < res.size(); j++) {
//         Plaintext res_pt;
//     vector<double> mm_res;
//     ckks_evaluator.decryptor->decrypt(res[j], res_pt);
//     ckks_evaluator.encoder->decode(res_pt, mm_res);
...
//     for (auto i = 0; i < 1; i++) {
//         average_err += fabs(mm_res[i]/2.0 - matrix_1x8_T_out[0][i]);
//     }

//     std::cout << "Average error: " << average_err / 1.0 << std::endl;
}
